...
import { fetchEnglishMetadata, generateChineseSummaryPrompt } from '@/lib/englishMetadataFetcher';
...
export default async function handler(
  ...
  try {
    console.log(`开始抓取英文文献元数据 - DOI: ${doi}, Title: ${title}`);

    // 1. 抓取英文元数据
    const metadata = await fetchEnglishMetadata({ title, doi });
    
    console.log('英文元数据抓取结果:', {
    ...
    if (metadata.title) {
      ...
      console.log('生成的中文概述:', chineseSummary);
    }
    ...
    console.error('英文元数据抓取失败:', error);
    return res.status(500).json({
      success: false,
      error: `英文元数据抓取失败: ${error}`
    });
  }
}
